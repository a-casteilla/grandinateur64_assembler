...
main ()
{
  ...
  struct symbol * imported_symbols = NULL;
  ...
  if (scopes) {
    imported_symbols = import_symbols("input.sym", 0x10000000, scopes);
    scope_of_lines(lines, scopes);
    symbols = find_labels(lines);
    symbols = symcat(symbols, imported_symbols);
    free(imported_symbols);

    eval_symbols = eval_def_once(lines, symbols);

    if (eval_symbols != NULL) {
      symbols = symcat(symbols, eval_symbols);
      free(eval_symbols);
    }
    ...
    for (struct symbol * s = symbols; s->name; s++) {
      ...
      printf("Is defined in scope with a level of %u\n", (s->scope)->level);
