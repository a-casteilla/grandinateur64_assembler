 ...
 */
bool
convert_str_num (const char * input,
                 uint64_t *   out)
{
  switch (*input) {
    case '0':
      if (strspn(input+1, "01234567") != strlen(input + 1)) {
    ...
    case '+':
    case '-':
      if (strspn(input+1, "0123456789") != strlen(input + 1)) {
        return true;
      }
      break;
    case '$':
      if ((strspn(input+1, "0123456789abcdefABCDEF") != strlen(input + 1))) {
        return true;
      }
      break;
    case '%':
      if ((strspn(input+1, "01") != strlen(input + 1))) {
        return true;
      }
      break;
    default:
